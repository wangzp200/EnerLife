...
namespace EnerlifeCN.Code.FormExt.Custom.COR0050
{
    public class COR0050 : SwBaseForm
    {
        ...
        private DBDataSource _ioDbCor0050, _ioDbCor0051;
        ...
        public override void FormCreate(string formUId, ref ItemEvent pVal, ref bool bubbleEvent)
        {
            ...
            _ioDbCor0050 = MyForm.DataSources.DBDataSources.Item("@COR0050");
            _ioDbCor0051 = MyForm.DataSources.DBDataSources.Item("@COR0051");
            Fieldlist.Add("@COR0051", "U_ItemCode");
            _ioDbCor0051.RemoveRecord(0);
            CommonUtil.MtxAddRow(_ioMtx, _ioDbCor0051, true);
        ...
        public override void ItemEventHandler(string formUId, ref ItemEvent pVal, ref bool bubbleEvent)
        {
                ...
                if (oCflEvento != null)
                {
                    ...
                    if (result.Rows.Count > 0)
                    {
                        if (pVal.ColUID == "C1")
                        {
                            try
                            {
                                _ioDbCor0051.Offset = oCflEvento.Row - 1;
                                _ioMtx.GetLineData(oCflEvento.Row);

                                _ioDbCor0051.SetValue("U_ItemCode", oCflEvento.Row - 1,
                                    result.GetValue("ItemCode", 0) as string);
                                _ioDbCor0051.SetValue("U_ItemName", oCflEvento.Row - 1,
                                    result.GetValue("ItemName", 0) as string);
                                _ioDbCor0051.SetValue("U_FromQty", oCflEvento.Row - 1, "1");
                                _ioDbCor0051.SetValue("U_ToQty", oCflEvento.Row - 1, "1");
                                ...
                                {
                                    CommonUtil.MtxAddRow(_ioMtx, _ioDbCor0051, false);
                                }
                ...
                currentRow != licRow)
            {
                ...
                _ioDbCor0051.Offset = licRow - 1;
                _ioMtx.GetLineData(licRow);
                var lsItemCode = _ioDbCor0051.GetValue("U_ItemCode", licRow - 1);
                    ...
                    case "C5":
                        if (!string.IsNullOrEmpty(value))
                        {
                            foreach (var entry in _validValueList)
                            {
                                ...
                                if (skey == value)
                                {
                                    _ioDbCor0051.SetValue("U_UomEntry", licRow - 1, svalue);
                                    _ioDbCor0051.SetValue("U_UoM", licRow - 1, value);
                                    break;
                                }
                            }
                        }
                ...
                pVal.ItemChanged)
            {
                ...
                _ioDbCor0050.SetValue("U_ChanName", 0, desc);
            }
        }
