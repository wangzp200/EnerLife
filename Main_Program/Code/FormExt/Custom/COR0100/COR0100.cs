...
namespace EnerlifeCN.Code.FormExt.Custom.COR0100
{
    public class COR0100 : SwBaseForm
    {
        ...
        private DBDataSource _ioDbCor0100, _ioDbCor0101;
        ...
        public override void FormCreate(string formUId, ref ItemEvent pVal, ref bool bubbleEvent)
        {
            ...
            validValues = _ioZone.ValidValues;
            ...
            _ioDbCor0100 = MyForm.DataSources.DBDataSources.Item("@COR0100");
            _ioDbCor0101 = MyForm.DataSources.DBDataSources.Item("@COR0101");
            Fieldlist.Add("@COR0101", "U_ItemCode");
            _ioDbCor0101.RemoveRecord(0);
            CommonUtil.MtxAddRow(_ioMtx, _ioDbCor0101, true);
        ...
        public override void ItemEventHandler(string formUId, ref ItemEvent pVal, ref bool bubbleEvent)
        {
                ...
                if (oCflEvento != null)
                {
                    ...
                    if (result.Rows.Count > 0)
                    {
                        if (pVal.ColUID == "C1")
                        {
                            try
                            {
                                _ioDbCor0101.Offset = oCflEvento.Row - 1;
                                _ioMtx.GetLineData(oCflEvento.Row);
                                _ioDbCor0101.SetValue("U_ItemCode", oCflEvento.Row - 1,
                                    result.GetValue("ItemCode", 0) as string);
                                _ioDbCor0101.SetValue("U_ItemName", oCflEvento.Row - 1,
                                    result.GetValue("ItemName", 0) as string);
                                _ioMtx.SetLineData(oCflEvento.Row);

                                if (oCflEvento.Row == _ioMtx.VisualRowCount)
                                {
                                    CommonUtil.MtxAddRow(_ioMtx, _ioDbCor0101, false);
                                }
                ...
                licRow != currentRow)
            {
                ...
                _ioDbCor0101.Offset = licRow - 1;
                _ioMtx.GetLineData(licRow);
                var lsItemCode = string.Empty;
                if (pVal.ColUID == "C5")
                {
                    lsItemCode = _ioDbCor0101.GetValue("U_ItemCode", licRow - 1);
                }
                    ...
                    case "C5":
                        if (!string.IsNullOrEmpty(value))
                        {
                            foreach (var entry in _validValueList)
                            {
                                ...
                                if (skey == value)
                                {
                                    _ioDbCor0101.SetValue("U_UomEntry", licRow - 1, svalue);
                                    _ioDbCor0101.SetValue("U_UoM", licRow - 1, value);
                                    break;
                                }
                            }
                        }
                ...
                pVal.ItemChanged)
            {
                ...
                _ioDbCor0100.SetValue("U_ZoneName", 0, desc);
            }
        }
